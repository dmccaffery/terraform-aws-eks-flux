...
* var.patchy, var.stack_components, workload_identity.patchy_egress_broker and the patchy-status entry of sso.clients are removed; modules/secrets is renamed modules/sso-secrets and drops stack_components, agent_harnesses, claude_provider and every patchy container; flux.sync.url now defaults to oci://<registry>/manifests/platform and requires platform-manifests (flux-manifests) >= 4.0.0. The ordered cut-over of a running cluster is documented under "Migrating from 3.x" in both READMEs.
* **artifact-store:** github.manifests_id is replaced by github.manifest_publishers; the platform publisher role is renamed from <name>-manifest-publisher to <name>-<repo>-publisher (moved blocks keep it tracked, the name change replaces it) - update the platform repo's AWS_MANIFEST_PUBLISHER_ROLE from the manifest_publishers output.
...
* **artifact-store:** one path-scoped manifest publisher per repo ([8afafee](https://github.com/dmccaffery/terraform-aws-eks-flux/commit/8afafee71b19c326f5fa8177cb6ae54be6cd3b47))
...
* generic cluster module with platform election and application seeds ([e01634d](https://github.com/dmccaffery/terraform-aws-eks-flux/commit/e01634dd2868cf490b4ac2766221ad0f93e08478))
...
* **deps:** update bitwise-media-group/github-workflows action to v6.3.0 ([#37](https://github.com/dmccaffery/terraform-aws-eks-flux/issues/37)) ([5200645](https://github.com/dmccaffery/terraform-aws-eks-flux/commit/5200645ca1ce7f455c29be15e35043fdf7f5409c))
* exclude terraform 1.16.0 from required_version ([8463102](https://github.com/dmccaffery/terraform-aws-eks-flux/commit/8463102a7cd08caf5de74d55cd32446d1c7304c3))
...
* **iam:** create replacement identities before destroying their predecessors ([688d09d](https://github.com/dmccaffery/terraform-aws-eks-flux/commit/688d09d9514ecebc3d4b719154b884a34e67382e))
...
* **node-group:** drop create_before_destroy so a cluster replacement can destroy first ([93434ce](https://github.com/dmccaffery/terraform-aws-eks-flux/commit/93434ce6de59f3fe725602f45d009e736f9a617f))
